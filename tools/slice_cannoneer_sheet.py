"""One-off slicer for assets/raw/gen_minions/r4_cannoneer_sheet.png (new Renaissance
'cannoneer' monster, replacing mortar_crew's slot in R4 - cf. TODO.md era-coherence audit).

Simpler than slice_weapon_sheets.py / slice_gladiator_sheets.py: this sheet has no caption text
to exclude, just 3 poses (neutral/windup/attack) stacked vertically on a pure chroma-green
background, so "content" = any non-green pixel (including bright white smoke in the attack pose,
which the caption-aware slicers would have wrongly excluded).
...
SRC = os.path.join(ROOT, 'assets', 'raw', 'gen_minions', 'r4_cannoneer_sheet.png')
DST = os.path.join(ROOT, 'assets', 'enemies', 'states')
NAMES = ['cannoneer_neutral', 'cannoneer_windup', 'cannoneer_attack']
